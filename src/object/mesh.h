...
public:
    mesh() {}
    mesh(const char* filename, shared_ptr<material> m, bool use_vn = false, bool use_vt = false)
        : mp(m) {
...
private:
    void create_mesh(const char* filename, bool use_vn, bool use_vt);
};
...
void mesh::create_mesh(const char* filename, bool use_vn, bool use_vt) {
...
//     }
//     smoothMesh(const hittable_list& triangles) {
